...
try:
    ...
    print(
        "Error: multi_agent_pipeline.py not found. Please ensure it's in the same directory."
    )
    sys.exit(1)
...
class NewsCollectionTool(BaseTool):
    ...
    description: str = (
        "Collect and categorize Bitcoin-related news from RSS feeds. Returns structured news data with short-term and long-term impact analysis."
    )
...
class NewsAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyze the effects of news on Bitcoin prices using trained models. Takes news data and returns probability distributions and market sentiment."
    )
    config: Dict[str, Any] = Field(default_factory=dict)
...
class PriceForecastTool(BaseTool):
    ...
    description: str = (
        "Forecast Bitcoin prices for the next 10 days based on news analysis. Returns detailed price predictions with confidence intervals."
    )
    config: Dict[str, Any] = Field(default_factory=dict)
...
class InvestmentAdvisoryTool(BaseTool):
    ...
    description: str = (
        "Generate comprehensive investment advisory based on complete analysis. Returns detailed investment recommendations with risk assessment."
    )
    config: Dict[str, Any] = Field(default_factory=dict)
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        super().__init__(
            config=config, recommendation_agent=RecommendationAgent(config)
        )
...
class BitcoinLangChainOrchestrator:
    ...
    def __init__(self, config_path: str = None):
        ...
        if not LANGCHAIN_AVAILABLE:
            raise ImportError(
                "LangChain is required but not installed. Install with: pip install langchain langchain-openai langchain-google-genai"
            )
        ...
        os.makedirs(
            self.config.get("output_dir", "outputs/langchain_results"), exist_ok=True
        )
    ...
    def _create_agent(self):
        ...
        if not self.tools:
            raise ValueError(
                "No tools initialized. Ensure tools are properly configured."
            )
        ...
        try:
            ...
            if "tool_names" in msg or "tools" in msg:
                logger.info(
                    "Falling back to create_react_agent without custom prompt to accommodate LangChain variant."
                )
                try:
                    ...
                    raise ValueError(
                        f"Failed to create LangChain agent (fallback also failed): {e2}"
                    )
            else:
...
def automated_mode(config_path: str = None, target_date: str = None):
    ...
    try:
        ...
        if results["status"] == "completed":
            ...
            print("-" * 40)
            print(
                results["response"][:1000] + "..."
                if len(results["response"]) > 1000
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Bitcoin Investment Advisory LangChain Agent"
    )

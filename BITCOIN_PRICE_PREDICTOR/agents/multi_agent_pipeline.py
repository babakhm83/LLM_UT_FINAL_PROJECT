...
class NewsAgent:
    ...
    def bucket_news(self, articles: List[NewsArticle]) -> Dict[str, List[NewsArticle]]:
        ...
        logger.info(
            f"NewsAgent: Bucketed - {len(short_term_articles)} short-term, {len(long_term_articles)} long-term"
        )

        return {"short_term": short_term_articles, "long_term": long_term_articles}

    def summarize_news_category(
        self, articles: List[NewsArticle], impact_type: str
    ) -> Dict[str, Any]:
...
class AnalysisAgent:
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            if model_path and os.path.exists(model_path):
                try:
                    ...
                    self.local_model = {"tokenizer": tokenizer, "model": model}
    ...
    def analyze_news_effects(self, news_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.local_model:
            logger.warning(
                "AnalysisAgent: No local model available, using mock analysis"
            )
    ...
    def run(self, news_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis_result = {**news_data, **btc_data, **effects_analysis}
...
class ForecastAgent:
    ...
    def forecast_prices(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.local_model:
            logger.warning(
                "ForecastAgent: No local model available, using mock forecast"
            )
...
class RecommendationAgent:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            if model_path and os.path.exists(model_path):
                ...
                logger.warning(
                    f"RecommendationAgent: Model path not found: {model_path}"
                )
        else:
    ...
    def generate_advisory(self, complete_data: Dict[str, Any]) -> str:
        ...
        if not self.local_model:
            logger.warning(
                "RecommendationAgent: No local model available, using simple advisory"
            )
...
class BitcoinPipelineOrchestrator:
    ...
    def _save_results(self, results: Dict[str, Any], target_date: str):
        ...
        output_path = os.path.join(
            self.config["output_dir"], f"bitcoin_pipeline_{target_date}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Bitcoin Investment Advisory Multi-Agent Pipeline"
    )
    parser.add_argument(
        "--config", type=str, help="Path to configuration JSON file (optional)"
    )
    parser.add_argument(
    ...
    if results["status"] == "completed":
        ...
        print(
            f"News Articles Processed: {results['pipeline_stages'][0]['data']['total_articles']}"
        )
        print("\nFINAL INVESTMENT ADVISORY:")
        print("-" * 40)
        print(
            results["final_advisory"][:500] + "..."
            if len(results["final_advisory"]) > 500
            else results["final_advisory"]
        )
        print("-" * 40)
        print(
            f"Full results saved to {orchestrator.config['output_dir']}/bitcoin_pipeline_{results['target_date']}.json"
        )
    else:

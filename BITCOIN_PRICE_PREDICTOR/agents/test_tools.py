    ...
    for tool_name, tool_class in tools:
        try:
            ...
            if agent_attr and hasattr(tool, agent_attr):
                ...
                print(
                    f"   Agent attribute '{agent_attr}' present: {type(attr).__name__}"
                )
            else:
                print(
                    f"   Agent attribute '{agent_attr}' not found on tool (available attrs: {list(tool.__dict__.keys())})"
                )

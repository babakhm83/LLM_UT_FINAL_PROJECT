...
try:
    ...
    print(
        "Google Generative AI package not available. Install with: pip install google-generativeai"
    )
...
try:
    ...
    print(
        "Transformers package not available. Install with: pip install transformers torch"
    )
...
DEFAULT_CONFIG = {
    ...
    "news": {
        ...
        "short_term_count": 10,
        "long_term_count": 10,
        "sources": ["http://feeds.feedburner.com/CoingeckoBuzz"],
    },
...
class BitcoinPipelineAgent:
    ...
    def _init_api_clients(self):
        ...
        if self.gemini_api_keys and GEMINI_AVAILABLE:
            ...
            logger.warning(
                "Gemini API keys provided but google-generativeai package not installed"
            )
        else:
        ...
        if self.openai_api_key:
            logger.warning(
                "OpenAI API key provided but OpenAI support has been removed. Using Gemini only."
            )
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            logger.warning(
                "Transformers not available, local models will not be loaded"
            )
    ...
    def _load_local_models(self):
        ...
        for model_name, path in model_paths.items():
            if path and os.path.exists(path):
                ...
                logger.warning(
                    f"{model_name} model path not found or not configured: {path}"
                )
    ...
    def collect_news(
        self, max_articles: int = None, target_date: str = None
    ) -> List[NewsArticle]:
        """
                ...
                for entry in parsed.entries:
                    try:
                        article = NewsArticle(
                            ...
                            source=(
                                feed_url.split("/")[2] if "/" in feed_url else feed_url
                            ),
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            scraped_articles = list(
                executor.map(self._scrape_article_content, unique_articles)
            )
    ...
    def _scrape_article_content(self, article: NewsArticle) -> NewsArticle:
        ...
        try:
            ...
            if news_article.text and len(news_article.text) > 100:
                article.content = news_article.text[:5000]
                if not article.published_date and news_article.publish_date:
                    article.published_date = news_article.publish_date.strftime(
                        "%Y-%m-%d"
                    )
                return article
    ...
    def bucket_news(self, articles: List[NewsArticle]) -> Dict[str, List[NewsArticle]]:
        ...
        logger.info(
            f"Bucketed articles: {len(short_term_articles)} short-term, {len(long_term_articles)} long-term"
        )

        return {"short_term": short_term_articles, "long_term": long_term_articles}

    def summarize_items(
        self, bucketed_news: Dict[str, List[NewsArticle]], style: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            short_term_summary = self._call_summarization_model(
                short_term_data, analysis_date, "short_term", style=style
            )

            long_term_summary = self._call_summarization_model(
                long_term_data, analysis_date, "long_term", style=style
            )
            ...
            news_analysis = {
                ...
                "high_impact_count": short_term_summary.get("high_impact_count", 0)
                + long_term_summary.get("high_impact_count", 0),
                "avg_confidence": (
                    short_term_summary.get("confidence", 0.5)
                    + long_term_summary.get("confidence", 0.5)
                )
                / 2,
                "key_events": short_term_summary.get("key_events", [])
                + long_term_summary.get("key_events", []),
                "daily_view": {
                    ...
                    "key_risks": short_term_summary.get("risk_factors", [])
                    + long_term_summary.get("risk_factors", []),
                    "watch_items": short_term_summary.get("watch_items", [])
                    + long_term_summary.get("opportunities", []),
                    "recommendation_short_term": {
                        "action": short_term_summary.get("recommendation", "HOLD"),
                        "probability": short_term_summary.get(
                            "recommendation_confidence", 0.5
                        ),
                    },
                    "recommendation_long_term": {
                        "action": long_term_summary.get("recommendation", "HOLD"),
                        "probability": long_term_summary.get(
                            "recommendation_confidence", 0.5
                        ),
                    },
                },
            ...
            try:
                ...
                try:
                    ...
                    effects_analysis = {
                        ...
                        "bear_prob": 0.25,
                        "base_prob": 0.10,
                        "scenarios": {"bullish": 0.65, "bearish": 0.25, "base": 0.10},
                    }

                logger.info("Successfully analyzed news effects with local model")
            except Exception as e:
                logger.warning(
                    f"Local analysis model failed: {e}. Using mock analysis."
                )
                effects_analysis = {
                    "bull_prob": 0.65,
                    "bear_prob": 0.25,
        ...
        if "forecast" in self.local_models:
            try:
                ...
                if len(prices) >= 10:
                    ...
                    next_10_day_prices = [
                        current_price * (1 + 0.005 * i) for i in range(10)
                    ]
                ...
                logger.warning(
                    f"Local forecast model failed: {e}. Using mock forecast."
                )
    ...
    def generate_investment_advisory(self, complete_analysis: Dict[str, Any]) -> str:
        ...
        if "advisory" in self.local_models:
            try:
                ...
                logger.info(
                    f"Successfully generated investment advisory with local model ({len(advisory)} chars)"
                )
                return advisory
            except Exception as e:
                logger.error(
                    f"Error generating investment advisory with local model: {e}"
                )
        ...
        if self.gemini_api_keys:
            try:
                ...
                logger.info(
                    f"Successfully generated investment advisory with Gemini ({len(advisory)} chars)"
                )
                return advisory
    ...
    def _create_investment_advisory_prompt(self, daily_data: Dict[str, Any]) -> str:
        ...
        short_term_summary = daily_data.get(
            "short_term_summary", "No short-term summary available."
        )
        long_term_summary = daily_data.get(
            "long_term_summary", "No long-term summary available."
        )
...
Make your analysis comprehensive, data-driven, and suitable for institutional investors managing significant Bitcoin allocations. Use the next 10-day price predictions to inform your near-term tactical recommendations while maintaining focus on long-term strategic positioning."""
    ...
    def _create_simple_advisory(self, daily_data: Dict[str, Any]) -> str:
        ...
        short_term_rec = daily_view.get("recommendation_short_term", {}).get(
            "action", "HOLD"
        )
        long_term_rec = daily_view.get("recommendation_long_term", {}).get(
            "action", "HOLD"
        )
...
"""
    ...
    def run(self, target_date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Bitcoin investment advisory pipeline completed successfully for {target_date}"
            )
    ...
    def _save_results(self, results: Dict[str, Any], target_date: str):
        ...
        output_path = os.path.join(
            self.config["output_dir"], f"bitcoin_advisory_{target_date}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Bitcoin Investment Advisory Pipeline Agent"
    )
    parser.add_argument(
        "--config",
        type=str,
        help="Path to configuration JSON file (optional, defaults to config.json)",
    )
    parser.add_argument(
        "--date", type=str, help="Target date for analysis (YYYY-MM-DD)"
    )
    ...
    if results["status"] == "completed":
        ...
        print("-" * 50)
        print(
            results["advisory"][:500] + "..."
            if len(results["advisory"]) > 500
            else results["advisory"]
        )
        print("-" * 50)
        print(
            f"Full results saved to {os.path.join(agent.config['output_dir'], f'bitcoin_advisory_{results['target_date']}.json')}"
        )
    else:
